...
private func dummy() {
    ...
    _ = NSLocalizedString("Move block %2$s from %1$s %3$s to %1$s %4$s", comment: "translators: accessibility text. %1: block moving direction (string). %2: location of block - row or order number (string). %3: current block position (number). %4: next block position (number)")
